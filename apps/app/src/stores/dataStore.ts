import type { CreateGroupInput, Group, UpdateGroupInput } from '@/lib/api/groups'
import type { CreateListInput, List, UpdateListInput } from '@/lib/api/lists'
import type { CreateTaskInput, Task, UpdateTaskInput } from '@/lib/api/tasks'
...
export const useDataStore = create<DataState>((set, get) => ({
  ...
  reorderGroups: async (orderedGroupIds) => {
    ...
    if (reorderedGroups.length !== previousGroups.length)
      return
  ...
  reorderLists: async (orderedListIds) => {
    ...
    if (reorderedLists.length !== orderedListIds.length)
      return
  ...
  reorderTasks: async (orderedTaskIds) => {
    ...
    if (reorderedTasks.length !== orderedTaskIds.length)
      return
