...
function getPresetFromRemindAt(
  ...
  if (hours === 9 && minutes === 0) return "at_time";
  if (hours === 8 && minutes === 55) return "5min";
  if (hours === 8 && minutes === 45) return "15min";
  if (hours === 8 && minutes === 30) return "30min";
  if (hours === 8 && minutes === 0) return "1hour";
    ...
    <div className="space-y-3">
      ...
      {permission === "unsupported" && (
        <div className="flex items-start gap-2 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-md text-sm text-amber-700 dark:text-amber-300">
          ...
          <span>{t("reminder.notSupported")}</span>
        </div>
      )}
      ...
      {permission === "granted" && (
        <>
          ...
          <Select value={value.preset} onValueChange={handlePresetChange}>
            <SelectTrigger>
              <SelectValue placeholder={t("reminder.selectPreset")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="none">{t("reminder.presets.none")}</SelectItem>
              {hasDate && (
                <>
                  <SelectItem value="at_time">{t("reminder.presets.atTime")}</SelectItem>
                  <SelectItem value="5min">{t("reminder.presets.5min")}</SelectItem>
                  <SelectItem value="15min">{t("reminder.presets.15min")}</SelectItem>
                  <SelectItem value="30min">{t("reminder.presets.30min")}</SelectItem>
                  <SelectItem value="1hour">{t("reminder.presets.1hour")}</SelectItem>
                  <SelectItem value="1day">{t("reminder.presets.1day")}</SelectItem>
                </>
              )}
              <SelectItem value="custom">{t("reminder.presets.custom")}</SelectItem>
            </SelectContent>
          </Select>
          ...
          {remindAt && (
            <p className="text-sm text-stone-500 dark:text-stone-400">
              {t("reminder.willRemindAt")} {formatReminderTime(remindAt)}
            </p>
          )}
